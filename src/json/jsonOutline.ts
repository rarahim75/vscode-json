...
export class JsonOutlineProvider implements vscode.TreeDataProvider<number> {
  ...
  getTreeItem(offset: number): vscode.TreeItem {
    ...
    if (valueNode) {
      ...
      const treeItem: vscode.TreeItem = new vscode.TreeItem(
        ...
        hasChildren ? (valueNode.type === "object" ? vscode.TreeItemCollapsibleState.Expanded : vscode.TreeItemCollapsibleState.Collapsed) : vscode.TreeItemCollapsibleState.None
      );
      treeItem.command = {
  ...
  copyPath(offset: number): void {
    const path = json.getLocation(this.text, offset).path;
    const mPath = path.map((p) => {
      return typeof p === "string" ? p : `[${p}]`;
    });

    console.log(mPath.join(".").replace(".[", "["));
    clipboardy.writeSync(mPath.join(".").replace(".[", "["));
  }
}
